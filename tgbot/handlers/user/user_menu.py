...
from tgbot.keyboards.inline_user import products_open_finl, products_confirm_finl
    ...
    if get_action == "yes":
        ...
        if 1 <= int(get_count) <= len(get_items):
            if get_user['user_balance'] >= amount_pay:
                save_items, save_count, save_len = buy_itemx(
                    get_items, get_count)

                if get_count != save_count:
                    amount_pay = round(
                        get_position['position_price'] * save_count, 2)
                    get_count = save_count

                update_userx(get_user['user_id'], user_balance=round(
                    get_user['user_balance'] - amount_pay, 2))
                add_purchasex(
